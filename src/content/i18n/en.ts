export const en = {
  nav: {
    home: "Home",
    portfolio: "Portfolio",
    services: "Services",
    contact: "Contact",
  },
  common: {
    learnMore: "Learn More",
    viewAll: "View All",
    viewProject: "View Project",
    getInTouch: "Get in Touch",
    startProject: "Start a Project",
    watchReel: "Watch Demo Reel",
    listenNow: "Listen Now",
    sendMessage: "Send Message",
    sending: "Sending...",
    messageSent: "Message sent successfully. We'll be in touch soon.",
    all: "All",
    backToTop: "Back to top",
    close: "Close",
    credits: "Credits",
    year: "Year",
    category: "Category",
  },
  home: {
    hero: {
      subtitle:
        "Original music and audio services for film, animation, theatre, games, and contemporary media.",
      ctaPrimary: "Explore Our Work",
      ctaSecondary: "Watch Demo Reel",
    },
    intro: {
      label: "Studio",
      title: "Where Cinematic Storytelling Meets Contemporary Production",
      body: "EON Resonance Lab is a creative music studio dedicated to storytelling through sound.\n\nWe create original music for film, animation, games, and contemporary media, combining cinematic composition, orchestral writing, and modern production.\n\nThrough original scoring, arrangement, and collaborative projects, we craft music that brings stories to life and leaves a lasting emotional impact.",
    },
    featured: {
      label: "Featured Work",
      title: "Selected Projects",
    },
    team: {
      label: "Studio",
      title: "Composers & Producers",
      subtitle:
        "Two distinct creative voices united by a shared commitment to storytelling through sound.",
    },
    cta: {
      title: "Ready to Score Your Next Project?",
      subtitle:
        "From orchestral film scores to K-pop production, we bring cinematic depth and contemporary polish to every collaboration.",
      button: "Start a Conversation",
    },
    demoReel: {
      label: "Demo Reel",
      title: "Hear Our Work",
    },
  },
  about: {
    studio: {
      label: "Studio",
      labelZh: "工作室简介",
      title: "Crafting Sound for the Stories of Tomorrow",
      body: "We believe every story deserves a sonic identity as compelling as its narrative. EON Resonance Lab exists at the intersection of cinematic composition, theatrical storytelling, and contemporary production — delivering original music that resonates across cultures and mediums.",
    },
    team: {
      label: "The Team",
      title: "Meet Our Composers",
    },
  },
  portfolio: {
    hero: {
      label: "Portfolio",
      title: "Our Creative Work",
      subtitle:
        "A curated selection of scores, arrangements, and productions across film, animation, and contemporary music.",
    },
    categories: {
      film: "Film",
      animation: "Animation",
      songs: "Songs",
      arrangements: "Arrangements",
    },
    empty: "No projects found in this category.",
  },
  services: {
    hero: {
      label: "Services",
      title: "What We Create",
      subtitle:
        "End-to-end music and audio services tailored for professional media productions.",
    },
    process: {
      label: "Our Process",
      title: "From Concept to Final Mix",
      steps: [
        {
          title: "Discovery",
          body: "Discuss project goals, creative direction, references, timeline, and artistic vision with the client.",
        },
        {
          title: "Composition",
          body: "Create original music, themes, arrangements, and initial drafts based on project requirements.",
        },
        {
          title: "Revision",
          body: "Work closely with the client through feedback rounds and revisions to refine the music until it matches the creative vision.",
        },
        {
          title: "Delivery",
          body: "Deliver the final approved audio files and project assets in the required formats.",
        },
      ],
    },
    cta: {
      title: "Let's Discuss Your Project",
      subtitle: "Tell us about your creative vision and we'll craft a tailored proposal.",
      button: "Request a Quote",
    },
  },
  projects: {
    hero: {
      label: "Projects",
      title: "Studio Pipeline",
      subtitle:
        "A transparent view of our completed work, active productions, and upcoming collaborations.",
    },
    status: {
      completed: "Completed",
      ongoing: "In Production",
      upcoming: "Upcoming",
    },
    sections: {
      completed: "Completed Projects",
      ongoing: "Ongoing Productions",
      upcoming: "Future Collaborations",
    },
  },
  contact: {
    hero: {
      label: "Contact",
      title: "Let's Create Together",
      subtitle:
        "Whether you're developing a feature film, animated series, game, or album — we'd love to hear about your project.",
    },
    form: {
      name: "Name",
      email: "Email",
      projectType: "Project Type",
      budget: "Budget Range",
      message: "Message",
      namePlaceholder: "Your name",
      emailPlaceholder: "you@studio.com",
      messagePlaceholder: "Tell us about your project, timeline, and creative vision...",
      projectTypes: [
        "Film Scoring",
        "Animation Scoring",
        "Game Music",
        "Musical Theatre",
        "Song Production",
        "Arrangement",
        "Orchestration",
        "Other",
      ],
      budgetRanges: [
        "Under ¥1,000 (~$150)",
        "¥1,000 – ¥3,000 (~$150–$450)",
        "¥3,000 – ¥5,000 (~$450–$700)",
        "¥5,000 – ¥8,000 (~$700–$1,150)",
        "¥8,000 – ¥10,000 (~$1,150–$1,450)",
        "Custom / To Be Discussed",
      ],
    },
    info: {
      title: "Studio Contact",
      emailLabel: "Email",
      locationLabel: "Location",
      socialLabel: "Follow Us",
      response: "We typically respond within 1–2 business days.",
    },
  },
  footer: {
    tagline: "Original music for film, animation, theatre, games & media.",
    rights: "All rights reserved.",
    navigation: "Navigation",
    connect: "Connect",
  },
  seo: {
    home: {
      title: "EON Resonance Lab — Music for Stories That Matter",
      description:
        "Premium music studio offering film scoring, animation music, game audio, musical theatre composition, and contemporary production.",
    },
    portfolio: {
      title: "Portfolio — EON Resonance Lab",
      description:
        "Explore our portfolio of film scores, animation music, game soundtracks, theatre compositions, and pop productions.",
    },
    contact: {
      title: "Contact — EON Resonance Lab",
      description:
        "Get in touch with EON Resonance Lab for your next film, animation, game, or music project.",
    },
  },
};
